...
def get_polygon_from_mask(mask, bbox):
    # Convert mask to binary (0s and 1s)
    mask_tensor = torch.from_numpy(mask).float()  # Convert NumPy array to PyTorch tensor
    mask_tensor = mask_tensor.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions

    # Determine output size based on the bounding box dimensions
    output_size = (int(bbox[3]), int(bbox[2]))  # (height, width)

    # Resize mask using torch.nn.functional.interpolate
    mask_resized = torch.nn.functional.interpolate(mask_tensor, size=output_size, mode='nearest')
    mask_resized = mask_resized.squeeze(0).squeeze(0).cpu().numpy()

    # Get contours from resized mask
    ...
    if contours:
        ...
        polygon = largest_contour.squeeze(1).tolist()
        return polygon
    else:
        return []
...
def mask_to_polygon(mask: np.array, report: bool = False) -> List[int]:
    ...
    return np.asarray(polygons,dtype="object").ravel().tolist()
    
def extract_coords(polygon):
    pairs_list = []
    # Iterate over the original list to extract pairs
    for i in range(0, len(polygon) - 1, 2):
        # Check if there are at least two elements remaining to form a pair
        if i + 1 < len(polygon):
            # Create a pair from consecutive elements
            pair = [polygon[i], polygon[i + 1]]
            # Append the pair to the pairs_list
            pairs_list.append(pair)
    return pairs_list
# polygons = mask_to_polygon(mask, report=True)
def calculate_bot_l(x,y,w,h):
    halfw = w/2
    halfh = h/2
    bot_l = (x-halfw,y+halfh)
    return bot_l
...
def draw_keypoints(output, image):
    output = non_max_suppression_kpt(output, 
                                    ...
                                    0.45, # IoU Threshold
                                    nc=model2.yaml['nc'], # Number of Classes
            ...
            if len(det2):
                ...
                for *xyxy, conf, cls in reversed(det2):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        # cv2.rectangle(im0, (xyxy[0], xyxy[1]), (xyxy[2], xyxy[3]), color=(255,) * num_channels)
                        # print(mask_bbox)
                        # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()
                        # mask_bbox = np.zeros(im.shape[:2], dtype=np.uint8)
                        # mask_bbox[(int(xywh[1])):(int(xywh[1]+xywh[3])), (int(xywh[0])):(int(xywh[0]+xywh[2]))] = 255
                        # print(mask_bbox)
                        label2 = f'{names2[int(cls)]} {conf:.2f}'
                        # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()
                        # plot_one_box(xyxy, im0, label=label2, color=colors2[int(cls)], line_thickness=1)
                        # draw_keypoints(output, frame, confidence, threshold)
                        nimg = im[0].permute(1, 2, 0) * 255
                        nimg = nimg.cpu().numpy().astype(np.uint8)
                        nimg = cv2.cvtColor(nimg, cv2.COLOR_RGB2BGR)
                        for idx in range(pred2.shape[0]):
                            plot_skeleton_kpts(nimg, pred2[idx, 7:].T, 3)
                        # bot_l = calculate_bot_l(xywh[0],xywh[1],xywh[2],xywh[3])
                        # bb1 = ((xyxy[2] - xyxy[0]) * (xyxy[3] - xyxy[1])).cpu()
                        # bb1_area = bb1.numpy()
                        # iou = bb1_area/area_px
                        # if iou <1:
                        #     print("inside")
                        # else:
                        #     print("outside")
